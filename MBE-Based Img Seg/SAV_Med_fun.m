%  function [phi_0,energy] = SAV_fun(phi_0,Img,pa)
function phi_0 = SAV_Med_fun(phi_0,Img,pa)
%FFT prepare
[M, N] = size(phi_0);
ii = 1:1:M;
ii = ii';
...
phi_lap2= fun_lap2(phi_0,pa);
eng_E1 = fun_E1(phi_0,Img,pa);
r = sqrt(eng_E1);
% energy=[r^2+1/2.*sum(sum(phi_0.*pa.delta.*pa.mu.*phi_lap2))];
b = fun_U_Med(phi_0,Img,pa)/r;
