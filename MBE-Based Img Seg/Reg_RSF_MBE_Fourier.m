...
function [phi, eng, eng_seg, eng_length, eng_mbe] =...
    Reg_RSF_MBE_Fourier(phi,Img,Ksigma,KI,KONE,nu,delta,dt,iterN,mu,lambda1,lambda2,epsilon, n)
...
for i=1:iterN
    ...
    %% MBE
    Tmbe = A_phi - phi_lap - delta*phi_lap2;
    ...
    dataForce=(lambda1-lambda2)*KONE.*Img.*Img+conv2(s1,Ksigma,'same')-2.*Img.*conv2(s2,Ksigma,'same');
     %dataForce=(lambda1-lambda2)*KONE.*Img.*Img...
        %+imfilter(s1,Ksigma,'replicate')...
        %-2.*Img.*imfilter(s2,Ksigma,'replicate');
    % eq.(15)
    ...
    figure(200);
    if mod(n, 100)==0
        subplot(2,2,3),imagesc(phi); colormap(jet); colorbar, axis equal; title(['phi, ' num2str(n)])
        hold on; contour(phi,[0 0],'r', 'LineWidth', 1);
        subplot(2,2,1),imagesc(A); colormap(jet);colorbar, axis equal; title(['A, ' num2str(n)]);
            hold on; contour(phi,[0 0],'r', 'LineWidth', 1);
        subplot(2,2,2),imagesc(L); colormap(jet);colorbar, axis equal; title(['L, ' num2str(n)]);
%             hold on; contour(phi,[0 0],'r', 'LineWidth', 1);
        subplot(2,2,4),imagesc(Tmbe); colormap(jet);colorbar, axis equal; title(['MBE, ' num2str(n)]);
            hold on; contour(phi,[0 0],'r', 'LineWidth', 1);  
    end
    
    abs_dataForce = abs(dataForce);
    %     [gfx, gfy] = gradient(dataForce);
    %     gfxy = sqrt(gfx.^2 + gfy.^2);
    %     dt = 1/max(abs_dataForce(:))
    % L
    ...
    %% Calculate Energy
    % Seg Term
    Hphi = double(phi>0);%0.5*(1 + 2/pi*atan(phi/epsilon));
    M1 = Hphi;
    M2 = 1 - Hphi;
    Seg1 = lambda1*(Img.^2.*M1.*KONE - 2*Img.*M1.*conv2(f1,Ksigma,'same') + M1.*conv2(f1.^2,Ksigma,'same'));
    Seg2 = lambda2*(Img.^2.*M2.*KONE - 2*Img.*M2.*conv2(f2,Ksigma,'same') + M2.*conv2(f2.^2,Ksigma,'same'));
    Seg = Seg1 + Seg2;
    
    % Length Term
    [gx, gy] = gradient(phi);
    gnorm = sqrt(gx.^2 + gy.^2);
    DrcPhi=(epsilon/pi)./(epsilon^2.+phi.^2);
    Length = nu*DrcPhi.*gnorm;
    ...
    lap = gxx + gyy;
    MBE1 = delta/2*lap.^2;
    MBE2 = 0.25*(gnorm.^2 - 1).^2;
    MBE = mu*(MBE1 + MBE2);
    
    eng_seg = [eng_seg, sum(Seg(:))/M/N];
    eng_length = [eng_length, sum(Length(:))/M/N];
    eng_mbe = [eng_mbe, sum(MBE(:))/M/N];
